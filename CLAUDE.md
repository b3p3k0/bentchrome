...
- **Free-for-all:** every vehicle (player and all AI) can damage every other — no teams. The only immunity is a shooter to its own fire; AI may engage any combatant. AI runs the player's full loadout at 3× mount cooldown (`WeaponMount.cooldown_scale`), line-of-sight gated.
