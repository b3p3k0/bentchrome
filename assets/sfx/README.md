...
| `sp_<special>` | that car's signature special fires | one per def file: `sp_` + the def's basename (`blunt_blaze.tres` → `sp_blunt_blaze`); Kandykane shares Hornet's `sp_molotov` via the shared def. `sp_taser`/`sp_blunt_blaze` **loop** while the effect runs; `sp_toe_jam` plays when the armed hit LANDS; `sp_red_glare` plays per rocket (×3) |
| `sp_placeholder` | any special fires without its own `sp_*` asset | "WHERE'S THE BEEF?" — formant-synthesized radio squawk; replace by dropping the real `sp_<special>` file, no code change |
| `env_genny` | the Ground Floor Gore generator dies (global, once) | deep muffled boom + arc sizzle — the yard goes dark |
| `env_siren` / `env_panic` / `env_chopper` | stage-event alerts (catalogued + rendered; triggers not yet wired) | police wail / crowd panic / huey liftoff — once-per-level, global |
| `announcer_<car>_wins` | campaign finale win (the stadium's rolling YOU WIN) | "<Carname> wins!" — baked PA speech per roster id, generated by `tools/synth_sfx.py announcer()` (espeak-ng on the dev box; the game ships only these oggs) |
| `announcer_<car>_loses` | any YOU LOSE (last life gone) | "<Carname> loses!" — same PA voice, a shade lower |
