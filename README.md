...
- Ammo: missiles are scavenged, not printed. Grab the green `M` (fire), cyan `H` (homing), orange `P` (power), and blue `R` (rear) crates around the arena; crates respawn, your patience won't. Rear missiles start at zero, just like mines. One P stash is behind something you'll have to break.
