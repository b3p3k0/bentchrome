...
| `SMASH_COUNT` | 3 | tutorial_director.gd | yard kills counted from the lesson-start snapshot, clamped to what's standing; ≥1 barrel unless none remain — a taste, not a chore; the rest is extra credit |
...
| announcer_<car> | 14 baked lines | end_screen._announce_winner (rolling win = finale-only) | "<Carname> wins!" — espeak-ng dev-bake + PA chain in synth_sfx.py; no runtime TTS; 0.8s after the sting |
